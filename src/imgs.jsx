import React from "react";
import "./App.css"; 
// // import chirag from './images/chirag.jpeg';
// // import akshat from './images/akshat.jpeg';
// // import arun from './images/arun.jpeg';
// // import aarushi from './images/aarushi.jpg';
import feature from './images/feature.png';

// const TheThought = () => {
//     return(
//       <div id="about">
//           <div className="heading-container">
//           <div id="line"></div><h1 id="creators">THE THOUGHT</h1><div id="line"></div>
//           </div>
//           <br/><br/><br/><br/>
//           <h1 id="whatthought">|  What we thought ?</h1> 
//           <br/><br/>
//           <p className="thoughtpara">This initiative is not merely a matter of individual carbon management <br/> 
//             but rather promises alignment of sustainability goals at local, national, and international levels. <br/>
//             Institutions are increasingly being asked to demonstrate their commitment toward sustainability, <br/> 
//             and so effective carbon tracking becomes a very valuable instrument in such endeavours. <br/><br/>
//             With this research, <br/>
//             this study is presented with successful case studies and best practices to bring the full potential of the GLOBON in transforming <br/>
//             the process of carbon emissions tracking into a powerful driver for positive environmental change.
//             <br/>
//             </p>
//             <br/><br/><br/><br/>
//       </div>
//     )
//   }


// const Creators = () => {
//   return(
//     <div id="connect">
//         <div className="heading-container">
//     <div id="line"></div><h1 id="creators">OUR CREATORS</h1><div id="line"></div>
//         </div>
//         <br/><br/><br/><br/>
//     <div className="creators">

//       <div id="chirag">
//         <img src={chirag} alt="chirag"/>
//         <label><b>Chirag Tiwari</b></label>
//         <p>Operation Manager</p>
//           <a href="https://www.linkedin.com/in/chirag-tiwari-9816a02a6" target="_blank" rel="noopener noreferrer">
//           <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-linkedin" viewBox="0 0 16 16">
//           <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
//           </svg>
//           </a>
//       </div>

//       <div id="akshat">
//         <img src={akshat} alt="akshat"/>
//         <label><b>Akshat Tiwari</b></label> 
//         <p>Project Developer</p>
//           <a href="https://www.linkedin.com/in/akshat-tiwari-092560213" target="_blank" rel="noopener noreferrer">
//           <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-linkedin" viewBox="0 0 16 16">
//           <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
//           </svg>
//           </a>
//       </div>

//       <div id="arun">
//         <img src={arun} alt="arun"/>
//         <label><b>Arun Patidar</b></label>
//         <p>Project Developer</p>
//           <a href="https://www.linkedin.com/in/arun-patidar-06b11a28b" target="_blank" rel="noopener noreferrer">
//           <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-linkedin" viewBox="0 0 16 16">
//           <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
//           </svg>
//           </a>
//       </div>

//       <div id="aarushi">
//         <img src={aarushi} alt="aarushi"/>
//         <label><b>Aarushi Chouhan</b></label>
//         <p>Social Manager</p>
//           <a href="https://www.linkedin.com/in/chirag-tiwari-9816a02a6" target="_blank" rel="noopener noreferrer">
//           <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-linkedin" viewBox="0 0 16 16">
//           <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
//           </svg>
//           </a>
//       </div>

//     </div>
//     </div>
//   )
// }

const Features = () => {
    return(
        <div className="homepage" id="Home">
        
              <div className="feat">
                <div className="icon">
                <img src={feature} alt="Globon" />
                </div>       
              </div>
            </div>
    )
};


export default Features;
// export default TheThought;
